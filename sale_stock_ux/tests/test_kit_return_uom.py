...
class TestKitReturnUom(TransactionCase):
    """Devolver un kit cuyo componente usa otra UdM que la línea de venta.

    La cantidad devuelta de un kit la calcula `_compute_kit_quantities` en la UdM
    de la línea, así que convertir los movimientos de los componentes no hace falta.
    """
    ...
    def test_return_of_kit_with_component_in_other_uom(self):
        """Validar la devolución de un kit computa la cantidad en la UdM de la línea."""
        # Precondición del caso: el movimiento no está en la UdM de la línea.
        self.assertNotEqual(self.component.uom_id, self.order_line.product_uom_id)
        self._validate(self.delivery)
